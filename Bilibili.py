...
import os
import re
# URL = https://www.bilibili.com/bangumi/play/ep693247?spm_id_from=333.337.0.0 必须是ep,其他的不行
init_url = input("Initial URL:")
...
# init_url = "https://www.bilibili.com/bangumi/play/ep693247?spm_id_from=333.337.0.0"
ep = re.search(r"\d+", init_url)
ep = ep.group()
...
name_obj = re.compile(rf"'share_copy': '《{name}》(?P<name>.*?)',")  # 番剧话名
...
for i in range(title):
    ...
    playurl = f"https://api.bilibili.com/pgc/player/web/playurl?support_multi_audio=true&avid={aid_list[i]}&cid={cid_list[i]}&qn=112&fnver=0&fnval=4048&fourk=1&ep_id={id_list[i]}&from_client=BROWSER&drm_tech_type=2"
    ...
    print(f"{name_title}(audio)", baseurl_audio, type(baseurl_audio))
    print(f"{name_title}(video)", baseurl_video, type(baseurl_video))
    print ("解析完成\n开始下载")
    # 视频
    with open(f"{name}/video/{name_title}.flv", mode="wb") as f:
        f.write(requests.get(baseurl_video, headers=download_header).content)
    print(f"{name_title}.flv下载完成")
    
    # 音频
    with open(f"{name}/audio/{name_title}.aac", mode="wb") as f:
